...
- **`fit-content` block elements do not auto-center within a grid column.** Setting `width: fit-content` on a `grid-column: 1 / -1` element constrains its width but leaves it left-aligned. Add `margin: 0 auto` alongside `width: fit-content` to center it within the full-width grid area.
