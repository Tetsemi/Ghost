# Implementation Contract — **Tracker** Ancestry Trait Comment + Index Patch (Per-Scene / Per-Session)  
*(Canonical, Updated, Non-Conflicting, Blank-Line Safe, Indent-Safe)*

> **Purpose:** Insert cadence-group INDEX comment blocks and per-tracker-item wiring-detail header comments for a **single ancestry** in the **Tracker** UI, while preserving existing formatting.  
...
### Definitions
- A tracker item is identified by the ancestry-scoped wiring pair:
  - `<input type="hidden" name="attr_show_<ancestry>_<traitKey>" ... />`
  - followed by `<div class="sheet-tracker-item sheet-tracker-<cadence> sheet-<ancestry>-<traitKey-dashed>"> ... </div>`
- `trackerCadence` is determined from the `.sheet-tracker-scene` / `.sheet-tracker-session` class.
- `dataCadence` is `ancestryDataMap.<ancestry>.traits.items.<traitKey>.usage_limit`, normalized to `scene|session|campaign|none`, with `usage_limit: "" = none`.
...
6. A **file-order table** with at least: sequence#, line#, traitKey, trackerCadence, dataCadence, nearest legacy header, issues.
...
2. Compute input fingerprint:
   - Size: `2809637`
   - SHA256: `E2A8B46DA5E9178D1557197EA605FD95A6C30332B3982093CECC12F74D33A3B3`
...
## 2) Ancestry Scope Gate (Tracker DOM Container Lock)
5. All DOM searches MUST be scoped to the **Tracker** ancestry container only (not ancestry descriptions, not non-tracker UI):
   - Find the **Tracker** section that contains the target ancestry’s tracker items.
   - Scope lock must be based on an **unambiguous ancestry-unique anchor** inside Tracker, e.g. the presence of:
     - `name="attr_show_<ancestry>_<traitKey>"` hidden show inputs, and/or
     - `.sheet-<ancestry>-<traitKey-dashed>` tracker item classes
6. If the tracker ancestry container is not found → **STOP**.
...
> **Note:** This contract intentionally targets **Tracker** markup (per-scene/per-session), not the ancestry trait description UI elsewhere.

---

## 3) Reference Pattern Lock (Non-Negotiable)
8. Treat the existing **Tracker Ancestry block that already has correct INDEX + wired headers** (the “known-good reference ancestry”) as the **required target shape** for:
...
*(If there is no existing ancestry tracker block with INDEX + wired headers, this gate fails: STOP and report “Missing reference pattern.”)*
...
### Data-side truth
- `TRAITS_IN_DATA`: from `ancestryDataMap.<ancestry>.traits`
- `CADENCE_IN_DATA`: distinct `usage_limit` values across those traits, normalized to:
  - `scene`, `session`, `campaign`, `` (only these strings)

### Tracker-side reality (scoped to the ancestry tracker container)
- `TRAITS_IN_TRACKER`: distinct `<traitKey>` detected from tracker wiring:
  - `name="attr_show_<ancestry>_<traitKey>"` (preferred), and/or
  - `name="attr_used_<cadence>_<ancestry>_<traitKey>"` (if present), and/or
  - `.sheet-<ancestry>-<traitKey-dashed>` (as a fallback identity check)

### Trackable coverage sets (scope-precise; prevents `usage_limit: ""/none` drift)
- `TRAITS_TRACKED_IN_DATA`: subset of `TRAITS_IN_DATA` where normalized `usage_limit ∈ { scene, session }`
  - (Include `campaign` in this set **only if** the Tracker UI for ancestries explicitly supports a campaign cadence list for that ancestry.)
- `TRAITS_UNTRACKED_IN_DATA`: subset of `TRAITS_IN_DATA` where normalized `usage_limit == none`

10. Require:
- `TRAITS_TRACKED_IN_DATA == TRAITS_IN_TRACKER` (exact match; trackable-only)
- AND `TRAITS_UNTRACKED_IN_DATA` must have **zero** tracker wiring rows:
  - No `attr_show_<ancestry>_<traitKey>` present in the tracker-scoped container for any `traitKey ∈ TRAITS_UNTRACKED_IN_DATA`
  - No `.sheet-<ancestry>-<traitKey-dashed>` tracker item present for any `traitKey ∈ TRAITS_UNTRACKED_IN_DATA`
- AND cadence presence:
  - If any trait in data has normalized `usage_limit === "scene"`, a **scene** tracker group must exist for that ancestry.
  - If any trait in data has normalized `usage_limit === "session"`, a **session** tracker group must exist for that ancestry.
  - If `campaign` is included in `TRAITS_TRACKED_IN_DATA`, a **campaign** tracker group must exist for that ancestry.
  - If `TRAITS_TRACKED_IN_DATA` is empty, the correct “PASS” behavior is to STOP with a report that “no tracker entries are required/allowed for this ancestry’s traits,” and generate no files.
  
11. If mismatch → **STOP** and report:
- Missing trait keys on each side (trackable-only comparison)
- Any untracked traits that incorrectly appear in Tracker
- Summary of detected tracker anchors (counts + examples)
- Where detected (data section location + tracker node summaries)

---

## 5) Source of Truth (Trait Set, Cost, Cadence)
12. Source of truth for:
- traits that exist
- `cost`
- cadence (`usage_limit`)
is **`ancestryDataMap.<ancestry>.traits`** in the **same input HTML file**.
...
14. A tracker “item” is identified by the wiring pattern (ancestry-scoped):
- A preceding show input:
  - `<input type="hidden" name="attr_show_<ancestry>_<traitKey>" />` *(or the exact in-file equivalent)*
- Followed by a tracker item container:
  - `<div class="sheet-tracker-item sheet-tracker-<cadence> sheet-<ancestry>-<traitKey-dashed>"> ... </div>`
- And a “used” checkbox attribute consistent with cadence:
  - `name="attr_used_<cadence>_<ancestry>_<traitKey>"` *(or exact in-file equivalent)*
...
16. `GROUP_NODE` = the **ancestry-scoped** tracker group region that contains the relevant cadence items.
17. If `GROUP_NODE` missing → **STOP**.
18. If cadence group has **zero** tracker items → **STOP**.

### Placement rules (strict)
19. Insert the INDEX block as the **first content inside** the cadence group container that directly contains the list of `.sheet-tracker-item.sheet-tracker-<C>` for that ancestry.
...
For every tracker item for the target ancestry (scene + session):

23. Anchor: find the exact tracker item container associated to `<traitKey>` (Section 6).
...
- ELSE:
  ...
  - Otherwise:
    ...
    - Then re-check nearest previous non-empty line:
      - If it is a legacy short header (single-line `<!-- ... -->` not INDEX and not Tracker Ancestry Trait:) → **delete that legacy line entirely**
...
`<!-- Tracker Ancestry Trait: {Display Name} | key: {key} | cost: {N} XP | cadence: {scene|session|campaign|none} | attrs: {show}, {used} | i18n: {name} -->`
...
- 1 title line
- `T` item lines (one per tracker item in that cadence group for that ancestry)
- 1 separator line
...
`<!-- - ancestry_<ancestry>_<traitKey> (<cost> XP, <cadence>) -->`

34. Ordering inside the cadence INDEX:
- Alphabetical by trait key (snake_case)
- Unless the known-good reference ancestry’s in-file index for that ancestry+cadence shows a different ordering; then match it.
...
36. Cadence purity:
- Every INDEX item’s trait key has `usage_limit === C` in data; no cross-cadence leakage.
...
## 12) Ancestry Selector Clause (Set Once Per Request)
47. `ANCESTRY_KEY = veyra` *(e.g., `feran`, `khadra`, `draevi`)*
